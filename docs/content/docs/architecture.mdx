---
title: Architecture
description: How flowx is structured as a set of agent skills and MCP tools.
---
...
- **Small jobs — inline.** `command="discover"`/`"migrate"` accept `adf_definitions` (relative export path → ARM JSON) or `airflow_definitions` (relative `.py` path → DAG source), supplied by the agent and materialized to a temporary directory; `package`/`migrate` return the DAB inline as `bundle = {"files": {relpath: text, …}}`. Inline data flows through the agent's context, so it is capped at approximately 5 MB.
- **Large sources — by reference (recommended).** Point the server at `adf_volume_path` or `airflow_volume_path` for a UC Volume read through the SDK Files API, or `adf_workspace_path` or `airflow_workspace_path` for a `/Workspace` directory read through the SDK Workspace API. Write the DAB to `output_volume_path` or `output_workspace_path`, returned as `bundle_uploaded`. The bytes bypass the agent; grant the app service principal read access on the source and write access on the output target.

Locally hosted, ordinary `adf_source_path` / `airflow_source_path` / `output_dir` paths and mounted volumes work as-is. For Airflow, `migrate` pauses before packaging when reconciled gaps remain, separates v1 leaf envelopes from structural or source-semantic gaps, and resumes from the reviewed `translation_report.agentic.json` only after the fingerprint-bound `resolve_agentic` workflow has applied a review decision. A reviewed report that intentionally retains placeholders packages with `status="completed_with_reviewed_gaps"`, while `completed` is reserved for a reviewed report with no remaining gap findings.
</Callout>
