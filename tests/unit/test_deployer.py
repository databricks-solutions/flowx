...
def _make_bundle(root, bundle_dir, *, jobs, deps=None, var_suffix=""):
    """Writes a minimal per-pipeline bundle: databricks.yml + one resource YAML.

    Args:
        jobs: resource keys (job names) this bundle defines.
        deps: resource keys of sibling jobs to reference via ${var.<dep><var_suffix>} in a run_job_task.
        var_suffix: appended to the variable name (use "_job_id" to mirror the real dab_writer naming;
            default "" uses a bare ${var.<dep>}). The deployer strips a _job_id[_<n>] suffix to recover
            the callee job key either way.
    """
    ...
    for job in jobs:
        tasks = []
        for dep in deps or []:
            tasks.append({"task_key": f"call_{dep}", "run_job_task": {"job_id": f"${{var.{dep}{var_suffix}}}"}})
...
class TestDiscovery:
    def test_reads_jobs_and_var_dependencies(self, tmp_path):
        ...
        # depends_on maps callee job key -> the ${var.<name>} variable name referencing it. The helper
        # emits a bare ${var.b} (no _job_id suffix), so key and var name coincide here.
        assert bundles["a"].depends_on == {"b": "b"}
        assert bundles["b"].depends_on == {}

    def test_job_id_suffixed_var_maps_to_callee_key(self, tmp_path):
        # dab_writer emits ${var.<callee>_job_id}; the deployer must map that back to callee key 'b'
        # (so ordering resolves) while remembering the real variable name 'b_job_id' for --var injection.
        _make_bundle(tmp_path, "a", jobs=["a"], deps=["b"], var_suffix="_job_id")
        _make_bundle(tmp_path, "b", jobs=["b"])
        bundles = {bundle.bundle_dir: bundle for bundle in _discover_bundles(tmp_path)}
        assert bundles["a"].depends_on == {"b": "b_job_id"}
...
def test_missing_databricks_cli_reports_cleanly(tmp_path, monkeypatch, capsys):
    # When `databricks` is not on PATH, subprocess.run raises FileNotFoundError; _run_cli must turn that
    # into a non-zero result with an actionable message so run() stops cleanly rather than tracebacks.
    _make_bundle(tmp_path, "a", jobs=["a"])

    def _boom(*_args, **_kwargs):
        raise FileNotFoundError(2, "No such file or directory", "databricks")

    monkeypatch.setattr(deployer.subprocess, "run", _boom)
    assert run(tmp_path) == 1
...
def test_missing_databricks_cli_does_not_block_dry_run(tmp_path, monkeypatch):
    # --dry-run never shells out, so even a totally broken subprocess.run must not stop it.
    _make_bundle(tmp_path, "a", jobs=["a"])
