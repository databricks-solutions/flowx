...
class TestWriteBundleGroup:
    ...
    def test_cross_group_call_becomes_var(self, tmp_path):
        # alpha calls gamma, which is NOT in this bundle -> rewrite to ${var.gamma_job_id}.
        wfs = [_workflow("alpha", ["gamma"])]
        write_bundle_group(wfs, tmp_path, bundle_name="alpha_only")
        alpha_yaml = (tmp_path / "resources" / "alpha.yml").read_text()
        assert "${var.gamma_job_id}" in alpha_yaml
        databricks_yml = yaml.safe_load((tmp_path / "databricks.yml").read_text())
        assert "gamma_job_id" in databricks_yml["variables"]
...
class TestLoadReportPipelinesShape:
    ...
    def test_load_report_handles_pipelines_format(self, tmp_path):
        ...
        workflows, skipped = _load_report(report_path)
    ...
    def test_load_report_skips_malformed_pipelines_entry(self, tmp_path):
        """A malformed ``"pipelines"`` entry is skipped (surfaced via the returned skip list, and later
        in SETUP.md) rather than dropping every pipeline; the valid pipelines still load."""
        report = {
            "pipelines": [
                {
                    "name": "pipeline_ok",
        ...
        workflows, skipped = _load_report(report_path)
        # The valid pipeline still loads; every offender is recorded in the skip list, not raised.
        assert {wf.name for wf in workflows} == {"pipeline_ok"}
        assert any("no_tasks_here" in label for label in skipped)
        assert any("index 2" in label for label in skipped)
        assert any("index 3" in label for label in skipped)

    def test_package_main_aborts_on_malformed_pipelines_entry(self, tmp_path, capsys):
        """``package`` aborts (non-zero, nothing written) when a pipeline entry is malformed.

        The source-reconciliation preflight fails closed on a malformed ``pipelines`` report before any
        bundle is written, so a corrupt/incomplete entry (here, one missing ``tasks``) is rejected with a
        clear message rather than silently emitting a bundle short a pipeline.
        """
        ...
        report = {
            "pipelines": [
                ...
                {"name": "no_tasks_here", "tags": {"source": "adf"}},  # missing "tasks"
            ]
        }
        report_path = tmp_path / "translation_report.json"
        report_path.write_text(json.dumps(report))
        ...
        exit_code = dab_main(
            ["--report", str(report_path), "--output-dir", str(out_dir), "--no-download-workspace-files"]
        )
        assert exit_code != 0
        assert "no_tasks_here" in capsys.readouterr().err
        # Fail-closed: nothing was written to the destination.
