...
#
#   PYTHONPATH="<plugin_root>/src" "<plugin_root>/.venv/bin/python" -m flowx.adapter inputs ingest
#
set -euo pipefail
...
Run the plugin's Python code with src/ on PYTHONPATH, for example:
  PYTHONPATH="$PLUGIN_ROOT/src" "$VENV_PYTHON" -m flowx.adapter inputs ingest
EOF
