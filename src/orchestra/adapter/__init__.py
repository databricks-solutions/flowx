...
* **Agent adapter** -- :mod:`flowx.adapter.session` plus the question
  shapes in :mod:`flowx.adapter.models`.  This is the layer an agent
...
* **Pipeline modifier** -- :mod:`flowx.adapter.operations`.  The
...
* :mod:`~flowx.adapter.models` -- StrEnums and dataclasses.
* :mod:`~flowx.adapter.operations` -- Free functions
  (``gather_questions``, ``apply_preferences``, ``validate_answer``,
  ``allowed_values_for``).
* :mod:`~flowx.adapter.constants` -- Question IDs, compute-mode
  strings, replacement names, and other shared constants.
* :mod:`~flowx.adapter.predicates` -- Pure IR predicates used by
  both ``operations`` and the bundler.
* :mod:`~flowx.adapter.session` -- The agent adapter class.
"""
