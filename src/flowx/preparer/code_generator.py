...
from flowx.models.source_types import FILE_SOURCE_TYPES, JDBC_SOURCE_TYPES, REST_SOURCE_TYPES
...
def _assemble_file_lookup_source_path(props: dict[str, Any]) -> str:
    ...
    # C-47 (LSC5-001): never bake a raw dataset() reference into the default path. lookup.translate
    # substitutes these; drop any that still leak through so spark.read doesn't get a broken
    # abfss://.../@dataset().fileName path.
...
def generate_web_activity_notebook(
    ...
    *,
    ...
    if auth:
        ...
        # C-38 (LSC4-002): prefer the preparer's resolved (scope, key) tuple; fall back to the legacy
        # (task_key, 'auth-credential') shape only when the preparer didn't compute one.
        ...
        if auth_type in ("MSI", "ManagedServiceIdentity"):
            # LSC3-002: MSI/Managed Identity has no static secret, so reading auth-credential would be a
            # placeholder that fails at runtime; raise NotImplementedError (web_activity's manual_credential
            # SetupTask already flags this in SETUP.md) so the user wires the exchange manually.
    ...
    if activity.method in ("POST", "PUT", "PATCH"):
        ...
        # Prefer the body the translator pre-resolved to Python code (the real TranslationContext lowered
        # @concat / @variables / @{...} that the empty context here could not).
        ...
        # Legacy: top-level Expression bodies stored directly on ``body``.
            ...
            # _resolve_body may return a JSON literal, a Python dict literal, or a repr()'d concat string;
            # parse strings as JSON when possible so requests.request(json=...) gets a real object.
...
def generate_set_variable_notebook(activity: SetVariableActivity) -> str:
    ...
    if activity.value_kind == "notebook_code" and activity.notebook_code:
        ...
        # Build the body as a lines list to avoid textwrap.dedent issues when import_block starts at
        # column 0 (which would stop dedent stripping the common leading whitespace).
        lines = ["import json"]
        if import_block:
...
def generate_copy_notebook(activity: CopyActivity, *, scope: str = "") -> str:
    ...
    # Hoist imports the body needs into a single cell at the top of the notebook, since some helpers
    # inline ``from datetime import ...`` at the call site (an awkward block between comment groups).
...
def _generate_sdp_copy_notebook(activity: CopyActivity, *, scope: str = "") -> str:
    ...
    body = textwrap.dedent(f"""\
        ...
        @sdp.table(
            name="{sink_table}",
            comment="Generated by flowx from ADF Copy activity '{activity.name}'.",
        )
...
def _notebook_header(title: str) -> str:
    ...
    return textwrap.dedent(f"""\
        ...
        # MAGIC # {title}
        # MAGIC
        # MAGIC *Auto-generated by flowx. Do not edit manually unless necessary.*
    """)
...
def _render_query_assignment(query: str) -> str:
    ...
    The flowx expression resolver may have pre-translated the original
...
def _render_sink_write(
    ...
    *,
    ...
    # File-format sink: write the format declared by the ADF output dataset. .save(path) skips the
    # metastore, matching the ADF semantic of a path-based dataset.
    if fmt and fmt != "delta":
        ...
        if volume_relative is not None:
            # Volume-rooted sink: the bundler sets output_path_root as a base_parameter (DAB-substituted
            # ${var.catalog}/${var.schema}). The dataset's folderPath/fileName @{...} expressions are
            # already rewritten to f-string fragments, so wrap the relative path in an f-string and join.
            rel_literal = volume_relative.replace('"', '\\"')
            preamble = ""
            # Pull in any modules the rewritten f-string fragments reference so the notebook runs as-is
            # (today only datetime, from @{formatDateTime(...)} rewrites).
        ...
        # No structured sink volume: fall back to a single output_path widget the user fills in. Common
        # when the linked service uses a masked connection string and no path can be reconstructed.
        return (
...
def _generate_autoloader_body(activity: CopyActivity) -> str:
    ...
    # Prefer the UC volume path when available (set by the copy preparer for external-volume setup);
    # otherwise fall back to the resolved abfss:// path or raw dataset path.
    ...
    # Use the volume (not /tmp) for checkpoints and schema-evolution storage so state persists across
    # cluster restarts and is visible in Unity Catalog.
...
def _generate_jdbc_body(activity: CopyActivity, *, scope: str = "") -> str:
    ...
    if is_expression:
        # The query is an ADF expression (e.g. @concat('SELECT * FROM ', item().schema_name, ...)); generate
        # a notebook that reads the current ForEach item from the "item" widget and builds the SQL dynamically.
        return (
            textwrap.dedent(f"""\
            import json
    ...
    *,
    ...
    lines.extend(
        [
            ...
            notes_list,
            "# MAGIC",
            "# MAGIC *Auto-generated by flowx motif collapser.*",
            "",
            "# COMMAND ----------",
            "",
        ]
