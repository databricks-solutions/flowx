...
from flowx.bundler.inner_job_params import (
...
def _resolve_for_each_inputs_with_bridge(
    ...
    if activity.inputs_bridge_notebook_code:
        ...
        notebook_source = render_bridge_notebook(
            ...
            value_key,
            title=f"ForEach inputs bridge: {activity.name}",
        )
    ...
    *,
    ...
    if len(inner_activities) == 1:
        ...
        if inner_prepared.extra_tasks:
            ...
            # LSC3-001: gather cluster hints from inner activities so the inner-job default cluster lifts
            # spark_env_vars / custom_tags / driver_node_type_id from the LS-derived cluster spec.
        ...
        # LSC3-001: gather cluster hints from every nested inner activity so the inner-job default
        # cluster picks up LS-derived spark_env_vars / custom_tags / driver_node_type_id.
        inner_cluster_hints = []
        for child in inner_activities:
