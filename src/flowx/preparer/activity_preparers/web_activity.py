...
if TYPE_CHECKING:
    from flowx.models.ir import WebActivity
...
def prepare(activity: WebActivity, *, scope: str = "") -> PreparedActivity:
    ...
    # C-38 (LSC4-002): thread any resolved AzureKeyVaultSecret (scope, key) into the generator so the
    # rendered dbutils.secrets.get uses real values, not the scope=task_key, key='auth-credential' fallback.
