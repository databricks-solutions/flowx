...
from flowx.models.ir import CopyActivity
from flowx.models.source_types import FILE_SOURCE_TYPES, JDBC_SOURCE_TYPES
...
from flowx.preparer.code_generator import generate_copy_notebook
...
def _lakeflow_connection_name_for_activity(activity: CopyActivity) -> str:
    ...
    Returns:
        A connection name namespaced under ``flowx_`` and derived
    ...
    return f"flowx_{_sanitize_identifier(linked_service_name)}_connection"
...
def _build_sink_volume_setup_task(activity: CopyActivity) -> SetupTask | None:
    ...
    return SetupTask(
        ...
        config={
            ...
            # location_type drives the storage-credential DDL the setup notebook emits (Azure MI /
            # S3 IAM / GCS service account); omitting it leaves the user a manual TODO.
