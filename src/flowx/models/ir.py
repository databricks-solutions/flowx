...
class MotifAnnotation:
    ...
    and each member activity also carries the same :attr:`Activity.motif_id` tag.
...
class Activity:
    ...
    # Parameter substitutions approximated at translation time (e.g. utcnow()); the bundler lists each
    # entry (widget_name/raw_expression/replacement/note) in SETUP.md.
    parameter_approximations: list[dict[str, str]] = field(default_factory=list)
    required_parameters: dict[str, str] = field(default_factory=dict)
    # Compute mode stamped by the modifier: serverless | classic_single_node | classic_multi_node | inherit | None.
...
class CopyActivity(Activity):
    ...
    # Code paradigm chosen by the modifier: "notebook" (PySpark) or "sdp" (Lakeflow SDP).
    target_format: str | None = None
    # True when the modifier selected Lakeflow Connect for an eligible database-source Copy -> Delta.
    use_lakeflow_connector: bool = False
    # LFC connector flavour when use_lakeflow_connector is True: "query_based" | "cdc" (None if unstamped).
...
class WebActivity(Activity):
    ...
    # Request body pre-lowered to a Python expression at translate time when it contained
    # @-expressions; the code generator emits it verbatim instead of re-resolving.
...
class MotifActivity(Activity):
    ...
    # Set by the modifier when the user opts into metadata-driven consolidation (access granted, size S/M).
    consolidate_metadata_driven: bool = False
    # Concrete lookup rows materialised at translation time; empty when consolidation was requested but
    # the values have not been resolved yet.
    lookup_values: list[dict[str, Any]] = field(default_factory=list)
    # Motif-specific settings from the collapsed activities (e.g. lookup_query/lookup_scope for
    # for_each_ingestion) so the notebook generator can fetch its own input list.
...
class TranslationReport:
    ...
    Attributes:
        ...
        detected_motifs: Multi-activity patterns the detector matched on the
            ...
            here are :class:`~flowx.models.motifs.DetectedMotif` instances.
    """
