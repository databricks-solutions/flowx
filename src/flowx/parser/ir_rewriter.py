...
string-typed field on every :class:`~flowx.models.ir.Activity`
...
(``original_activities`` on :class:`~flowx.models.ir.MotifActivity`)
...
def rewrite_pipeline_expressions(
    ...
    *,
    ...
    Returns:
        ...
        :func:`~flowx.parser.expression_parser.resolve_interpolated_string`.
...
def _rewrite_activity(activity: Activity, context: TranslationContext, warnings: list[str]) -> Activity:
    ...
    for field_info in dataclasses.fields(activity):
        if field_info.name in _FIELDS_TO_SKIP:
            continue
        original = getattr(activity, field_info.name)
        rewritten = _rewrite_value(
            original,
            context,
            warnings,
            field_path=f"{type(activity).__name__}.{activity.task_key}.{field_info.name}",
        )
        if rewritten is not original:
            field_overrides[field_info.name] = rewritten
...
def _rewrite_value(value: Any, context: TranslationContext, warnings: list[str], *, field_path: str) -> Any:
    ...
    if isinstance(value, SwitchCase):
        ...
        new_activities = [_rewrite_activity(inner_activity, context, warnings) for inner_activity in value.activities]
