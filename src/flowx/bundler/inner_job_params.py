"""Collects and normalizes parameters for ForEach inner jobs."""
...
def collect_inner_job_params(
    ...
    *,
    ...
    for name in sorted(param_names):
        # C-06: variables with a known setter task on the parent job route via {{tasks.X.values.Y}},
        # so they must NOT show up as inner-job parameter declarations.
    ...
    # Map bare @item() -> {{input}}, @item().field -> {{input.<field>}}, and pipeline params/variables
    # -> {{job.parameters.<name>}}.
...
def _scan_tasks(
    ...
    *,
    ...
    """
    field_name_kwargs: dict[str, Any] = {"item_field_names": item_field_names, "variable_names": variable_names}
    for task in tasks:
        notebook_task = task.get("notebook_task", {})
        ...
        condition_task = task.get("condition_task", {})
        if condition_task:
            _extract_refs(condition_task.get("left", ""), param_names, **field_name_kwargs)
            _extract_refs(condition_task.get("right", ""), param_names, **field_name_kwargs)
            _scan_tasks(condition_task.get("if_true", []), param_names, **field_name_kwargs)
            _scan_tasks(condition_task.get("if_false", []), param_names, **field_name_kwargs)

        for_each_task = task.get("for_each_task", {})
        body = for_each_task.get("task")
        if body:
            _scan_tasks([body], param_names, **field_name_kwargs)
