...
from flowx.models.dab import DabNotebook, ParameterApproximation, SecretInstruction
...
class Prereqs:
    ...
    # VAREX3-003: variables mutated inside a ForEach inner-job that a sibling reads; each entry is the
    # SetupTask.config dict ({variable_name, parent_foreach, message}).
    manual_variable_rollups: list[dict[str, Any]] = field(default_factory=list)
    # C-28 (NB-ITER4-001): notebook activities whose ADF notebookPath is an unresolved runtime expression;
    # each entry is the SetupTask.config dict ({task_key, activity_name, expression, widget_name}).
    dynamic_notebook_dispatches: list[dict[str, Any]] = field(default_factory=list)
    # C-30 (NB-ITER4-003): library jar/whl paths unresolved to a literal/dab_ref; each entry is the
    # SetupTask config dict ({task_key, library_type, expression, missing}).
    unresolved_libraries: list[dict[str, Any]] = field(default_factory=list)
    # C-33 (VAREX4-001/CF4-003): SetVariable activities whose ADF expression couldn't be lowered; each
    # entry is the SetupTask config dict ({task_key, variable_name, expression}).
...
def build_prereqs(
    *,
    ...
    # LSC3-006: union notebook-scanned secrets with the workflow's typed SecretInstruction list so
    # SETUP.md Option A (scope/key checklist) and Option B (create_secrets.py) reference the same
    # (scope, key) set. De-dupe by scope; later additions don't overwrite earlier values.
...
def render_setup_md(prereqs: Prereqs, *, bundle_name: str) -> str:
    ...
    if prereqs.missing_notebooks:
        ...
        lines.append(
            ...
            "flowx could not download the source (either no workspace path was "
        ...
        lines.append(
            ...
            "have flowx re-ingest it by running `databricks workspace export` and "
    ...
    if prereqs.manual_parameters:
        ...
        lines.append(
            ...
            "at existing workspace notebooks, so flowx cannot inject the computation; **update "
    ...
    if prereqs.parameter_approximations:
        ...
        lines.append("")
        lines.append(
            "flowx mapped the ADF expressions below to Databricks dynamic value "
    ...
    if prereqs.manual_variable_inits:
        ...
        lines.append(
            ...
            "translator couldn't lower.  flowx blanked the variable's "
    ...
    if prereqs.manual_credentials:
        ...
        lines.append(
            ...
            "direct Databricks equivalent.  flowx defaulted the bundle's "
    ...
    if prereqs.neutralized_conditions:
        ...
        lines.append(
            ...
            "`run_job_task` boundaries, so flowx blanked the operand. A blanked "
