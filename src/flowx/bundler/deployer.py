...
# Recover the callee's job resource key from the variable name by stripping the `_job_id[_<n>]` suffix.
# Names without that suffix are returned unchanged (tolerates the older bare `${var.<callee>}` form).
_VAR_JOB_ID_SUFFIX = re.compile(r"_job_id(?:_\d+)?$")


def _callee_key_from_var(var_name: str) -> str:
    """Maps a cross-bundle variable name (``<callee>_job_id``) back to the callee's job resource key."""
...
class DiscoveredBundle:
    ...
    Attributes:
        ...
        depends_on: Maps each cross-bundle callee's job resource key -> the ``${var.<name>}`` variable
            name that references it (``<callee>_job_id``). Deploy resolves ordering on the callee key
            and injects ``--var <name>=<id>`` using the variable name.
    """

    __slots__ = ("bundle_dir", "resource_keys", "depends_on")

    def __init__(self, bundle_dir: str, resource_keys: set[str], depends_on: dict[str, str]) -> None:
        ...
        for resource_yml in sorted(resources_dir.glob("*.yml")):
            ...
            for job_id in _iter_run_job_ids(doc):
                ...
                if match:
                    ...
                    depends_on[_callee_key_from_var(var_name)] = var_name
...
def run(
    ...
    *,
    ...
    for position, bundle_dir in enumerate(order, start=1):
        ...
        # Only pass the vars this bundle references. Keyed by the actual bundle-variable name
        # (``<callee>_job_id``) so ``--var <name>=<id>`` matches the declaration in databricks.yml, with
        # the id looked up by the callee's job resource key.
        needed = {
            var_name: deployed_ids[callee]
            for callee, var_name in sorted(bundle.depends_on.items())
            if callee in deployed_ids
        }
