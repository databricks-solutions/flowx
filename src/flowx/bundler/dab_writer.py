...
class MalformedReportError(Exception):
    """A translation report contained an entry flowx could not have produced.

    Reports are machine-generated by ``engine.py`` (``_pipeline_to_dict`` always
    emits ``name`` + ``tasks``), so a non-conforming entry signals corruption or
    an internal bug. Raising -- rather than silently dropping the entry -- keeps
    ``package`` from emitting a bundle that is quietly missing a pipeline.
    """
...
def write_bundle(
    ...
    """
    return write_bundle_group([workflow], output_dir, catalog=catalog, schema=schema, bundle_name=bundle_name)
...
def write_bundle_group(
    ...
    # matters because several steps below rewrite the task dicts in place — _namespace_bundle_artifacts
    # and _rewrite_cross_bundle_run_job_refs, the latter turning ${resources.jobs.X.id} into ${var.X}
    # and thereby erasing the Run Pipeline edges pipeline_graph reads. Without the copy, correctness of
    # the caller's dependency graph / DEPLOY.md order would silently depend on it being computed before
    # this call (main() does, but that is an unenforceable ordering trap). Copying makes the writer a
    # pure sink: callers can build the graph before or after, in any order.
    ...
    # this bundle) into ${var.X}, registering each so _build_databricks_yml declares the variable. The
    # "known" set is every pipeline + inner job key in the group, so intra-group calls stay as direct
    # ${resources.jobs.X.id} refs. Must run before databricks.yml / resource YAML are written below, or
    # the ref points at a non-existent resource node and `bundle deploy` fails ("no such node ...").
    known_bundle_jobs: set[str] = set()
    for workflow in workflows:
        known_bundle_jobs.add(normalize_task_key(workflow.name))
        known_bundle_jobs.update(normalize_task_key(inner.name) for inner in workflow.inner_workflows)
    for workflow in workflows:
        _rewrite_cross_bundle_run_job_refs(workflow.tasks, known_bundle_jobs, _cross_bundle_variables)
        for inner in workflow.inner_workflows:
            _rewrite_cross_bundle_run_job_refs(inner.tasks, known_bundle_jobs, _cross_bundle_variables)
    ...
    # 3. Write generated notebooks (union of every workflow's; dedupe drops repeats of the same path,
    #    e.g. a notebook shared by a workflow and its inner ForEach job — see _dedupe_notebooks).
    ...
    if generated_notebooks:
        created_files.extend(write_notebooks(generated_notebooks, src_dir))
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Build the Run Pipeline dependency graph once and thread it into both grouping (per-group inferred)
    # and DEPLOY.md rendering, so the task trees are scanned only once. write_bundle_group works on a
    # deep copy of its input, so it no longer matters whether this runs before or after the write loop
    # (it used to: the writer rewrote ${resources.jobs.X.id} -> ${var.X} in place, erasing the edges);
    # computing it here just avoids re-deriving the same graph twice.
    ...
    # Records what landed in each bundle dir so DEPLOY.md can describe the layout and deploy order.
    written_groups: list[tuple[str, list[PreparedWorkflow]]] = []
    for index, (group_name, group_workflows) in enumerate(groups):
        bundle_dir = args.output_dir if single_bundle else args.output_dir / group_name
        # Name the bundle after group_name (e.g. "flowx_bundle" for a multi-pipeline single bundle), not
        # the first pipeline: _group_workflows already folded --bundle-name into group_name, so falling
        # back to args.bundle_name here would let bundle.name resolve to workflows[0].name and make the
        # workspace path .bundle/<first pipeline>/dev — misleading for a bundle holding many pipelines.
        effective_bundle_name = group_name
        try:
            ...
            # e.g. the inner-ForEach-key collision guard in _namespace_bundle_artifacts. Surface it as a
            # clean message + exit code, consistent with the _group_workflows guard above, rather than a
            # raw traceback.
            print(f"Error: {exc}", file=sys.stderr)
            return 1
        all_created.extend(created)
        written_groups.append((group_name, group_workflows))
    ...
    if workflow.parameters:
        ...
        for parameter in workflow.parameters:
            ...
            if default is not None:
                ...
                # time) get an empty string so `bundle deploy` doesn't reject the missing field. Record it
                # so SETUP.md surfaces the synthetic default (rather than silently substituting "") — an
                # operator needs to know the value was caller-supplied in ADF and a run that forgets to
                # override it will see "" instead of failing fast.
                entry["default"] = ""
...
def _load_report(report_path: Path) -> tuple[list[PreparedWorkflow], list[str]]:
    ...
    if "pipelines" in report and isinstance(report["pipelines"], list):
        ...
        malformed: list[str] = []
        for index, pipeline_entry in enumerate(report["pipelines"]):
            if isinstance(pipeline_entry, dict) and "tasks" in pipeline_entry and "name" in pipeline_entry:
                workflows.append(_pipeline_dict_to_workflow(pipeline_entry))
            else:
                # Collect every offender rather than dropping it silently: this report is
                # machine-generated, so a non-conforming entry is corruption / an internal bug, and
                # skipping it would emit a bundle quietly missing a pipeline while ``package`` exits 0.
                name = pipeline_entry.get("name") if isinstance(pipeline_entry, dict) else None
                malformed.append(repr(name) if name else f"index {index}")
        if malformed:
            raise MalformedReportError(
                "Malformed pipeline entr"
                + ("y" if len(malformed) == 1 else "ies")
                + f" in report (expected a dict with 'name' and 'tasks' keys): {', '.join(malformed)}."
            )
        return workflows
