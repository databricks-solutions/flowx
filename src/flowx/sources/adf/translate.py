...
from flowx.sources.adf.loader import classify_activity, load_adf_definitions
from flowx.sources.adf.translators import (
    append_variable,
    copy,
    databricks_job,
    delete,
...
def translate_pipeline(
    ...
    *,
    ...
    Notes:
        ...
        :func:`~flowx.sources.adf.ir_rewriter.rewrite_pipeline_expressions`
    ...
    # variables backed only by a literal default (which never populate variable_value_cache); else the
    # false branch is dead code.
    ...
    # C-05 (VAREX-002): synthesise init SetVariable tasks for variables with a defaultValue, seeding
    # variable_cache so @variables('X') resolves to the init task's value instead of a self-referential dangler.
    init_variable_activities, context, synthesized_init_keys = _build_variable_init_activities(pipeline, context)
    ...
    # Collect every agentic/unsupported activity across the whole tree (including IfCondition/ForEach/Until
    # children) so each gap reaches the agent with its full ADF/ARM JSON.
    ...
    pipeline_ir = Pipeline(
        ...
        schedule=schedule,
        synthesized_variable_init_keys=synthesized_init_keys,
    )
...
def _declare_referenced_globals_as_bundle_variables(
    ...
    """
    referenced_names = set(_VAR_REF_RE.findall(json.dumps(ir_serde.pipeline_to_dict(pipeline_ir), default=str)))
    declarations = {
        name: {
...
def _filter_motifs_for_collapse(
    ...
    Args:
        detected_motifs: Output of
            :func:`flowx.motifs.detector.detect_motifs`.
    ...
    Returns:
        ...
        :func:`flowx.motifs.collapser.collapse_motifs`.
    """
    # activity_and_notify is destructive (drops the notify activities for job-task notifications), so it's
    # never collapsed implicitly -- only when the user opts into a notification destination.
    if motif_consolidations is None:
        return [motif for motif in detected_motifs if motif.definition.motif_id not in _OPT_IN_ONLY_MOTIFS]
    return [motif for motif in detected_motifs if motif_consolidations.get(motif.definition.motif_id) == "consolidate"]
...
# C-10 (SCHED-001): map the Windows timezone names ADF emits onto the IANA names DAB's
# schedule.timezone_id expects. Only corpus-observed ones are mapped; anything else passes through.
...
def _compile_pipeline_schedule(
    ...
    matching_triggers = [trigger for trigger in triggers if _trigger_references(trigger, pipeline_name)]
    if not matching_triggers:
        return None

    # First matching trigger wins -- ADF allows multiple triggers per pipeline but DAB schedules are 1:1;
    # downstream tooling can surface the rest via SETUP.md.
    trigger = matching_triggers[0]
    spec = _adf_trigger_to_schedule(trigger)
    if spec is not None:
        # SCHED3-003: pull per-pipeline parameter overrides off the matching pipelineReference so
        # trigger-injected params propagate to the job's default parameter values.
...
def _adf_trigger_to_schedule(trigger: Any) -> dict[str, Any] | None:
    ...
    if trigger_type == "ScheduleTrigger":
        ...
        # SCHED3-002: Day/Week/Month with interval > 1 can't be expressed in quartz cron without
        # enumerating every Nth occurrence; use the trigger.periodic primitive instead.
        ...
        if periodic is not None:
            ...
            return spec
        # C-45 (SCHED5-002): interval > 1 Month has no monthly-cron form (cron ignores the interval) and
        # the DAB periodic enum has no MONTHS unit, so surface a manual setup note instead of a wrong cron.
...
def _recurrence_to_periodic(recurrence: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not isinstance(interval, int) or interval <= 1:
        return None
    # C-45 (SCHED5-002): the DAB periodic enum only has DAYS/HOURS/WEEKS, so month frequencies route to the
    # quartz cron path (monthDays); an interval > 1 Month (not monthly-cron-expressible) is a setup note.
...
def _recurrence_to_quartz_cron(recurrence: dict[str, Any]) -> str | None:
    ...
    # C-44 (SCHED5-001): when the schedule has no explicit time-of-day, derive hour/minute from startTime
    # (ADF's default); reading only schedule.minutes/hours would shift a 21:00 startTime to midnight.
    ...
    if frequency == "Week":
        days = ",".join(_DAYS_OF_WEEK_MAP.get(day, day) for day in week_days) or "MON"
...
def _build_variable_init_activities(
    pipeline: AdfPipeline,
    context: TranslationContext,
) -> tuple[list[Activity], TranslationContext, frozenset[str]]:
    ...
    Also returns the exact set of synthesised init task keys so downstream
    pruning can identify these tasks explicitly and never act on a
    customer-authored task.
    """
    ...
    for var_name, var_def in pipeline.variables.items():
        ...
        if expr_result is not None:
            ...
            # VAREX3-002: Boolean defaults render lowercase 'true'/'false' so @equals(variables('continue'),
            # true) matches ADF; Python str(True) would emit 'True' and silently invert the comparison.
            if isinstance(default, bool):
        ...
        init_keys.add(task_key)
        # Register the synthesised setter so @variables('X') resolves to {{tasks._init_X.values.X}}; when
        # the value is itself a DAB ref (e.g. @utcNow()), inline it directly.
    ...
    return init_tasks, context, frozenset(init_keys)
...
def _map_dependency_conditions(conditions: list[str] | None) -> str | None:
    ...
    if not conditions:
        return None
    normalized = [condition for condition in conditions if condition]
    if not normalized:
        return None
    if len(normalized) == 1:
        return normalized[0]
...
def _resolve_ls_parameters(
    ...
    if isinstance(activity_supplied, dict):
        for pname, pval in activity_supplied.items():
            ...
            # C-03: route @-prefixed activity-supplied values through the expression parser so
            # @pipeline().globalParameters.X collapses to the factory value when one is set.
            if context is not None and isinstance(raw, str) and raw.startswith("@"):
                result = resolve_expression(raw, context)
                # C-13 (NB-ITER3-002/LSC3-003/VAREX3-006): accept both literal and dab_ref so
                # @pipeline().parameters.X collapses to {{job.parameters.X}} (valid in custom_tags values).
...
def _unwrap_expression_value(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        ...
        # Some payloads omit the type marker but share the shape; conservatively unwrap only when the dict
        # has exactly {value, type} so we don't corrupt nested config like {"workspace": {"destination": ...}}.
        if set(value.keys()) == {"value", "type"}:
            return _unwrap_expression_value(value["value"])
        return {k: _unwrap_expression_value(v) for k, v in value.items()}
    if isinstance(value, list):
        return [_unwrap_expression_value(item) for item in value]
    return value
...
def _substitute_ls_params(value: Any, params: dict[str, Any]) -> Any:
    ...
    if isinstance(value, list):
        return [_substitute_ls_params(item, params) for item in value]
    return value
...
def _extract_cluster_config(
    ...
    # C-02 (NB-ITER2-2/LSC2-003): unwrap any {value, type:'Expression'} dicts that survived substitution -
    # map fields like custom_tags / spark_env_vars must be plain Map[String, String] for the cluster YAML.
    ...
    # C-39 (LSC4-004): capture the ADF auth shape (e.g. MSI / a CredentialReference) so the bundler emits
    # a manual_credential SetupTask noting single_user_name was rewritten to the deploying user.
...
def _pipeline_to_dict(pipeline: Pipeline) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "tags": pipeline.tags,
        "tasks": _prune_dead_variable_inits(
            [_activity_to_dict(task) for task in pipeline.tasks],
            pipeline.synthesized_variable_init_keys,
        ),
    }
...
def _prune_dead_variable_inits(
    tasks: list[dict[str, Any]],
    synthesized_init_keys: frozenset[str],
) -> list[dict[str, Any]]:
    """Drop synthesised variable-init tasks that nothing references.

    ``_build_variable_init_activities`` emits an init SetVariable task for
    every declared variable carrying a ``defaultValue`` so that a read of the
    variable always has a concrete writer task to bind to.  When an explicit
    ADF ``SetVariable`` activity dominates every read of that variable, the
    translator re-points all reads at the explicit setter and the init task
    becomes dead weight — it still runs on every execution and, sharing the
    same ``key``, reads as a confusing duplicate of the real setter.

    ``synthesized_init_keys`` is the exact set of task keys the translator
    generated for these init tasks; only those keys are eligible for pruning,
    so a customer-authored task is never removed.

    This sweep runs after translation and expression rewriting are complete,
    so every surviving reference is final.  A synthesised init task is kept
    only if its task key still appears in the *other* tasks under one of the
    two real reference forms: an anchored task-value token
    (``tasks.<key>.values``) or a structured ``depends_on`` edge.  Matching
    those forms rather than a bare substring against the serialised IR avoids
    a false keep when the key text merely appears inside an unrelated key or
    path (e.g. a sibling init ``_init_x_2``, or a notebook path
    ``/Shared/_init_x_report`` colliding with ``_init_x``).  Removing a task
    that is referenced nowhere can never change behaviour, so this is safe.
    """
    init_keys = {t["task_key"] for t in tasks if t.get("task_key") in synthesized_init_keys}
    if not init_keys:
        return tasks

    live_keys: set[str] = set()
    for t in tasks:
        if t.get("task_key") in init_keys:
            continue
        blob = json.dumps(t)
        for key in init_keys:
            if f"tasks.{key}.values" in blob or any(dep.get("task_key") == key for dep in (t.get("depends_on") or [])):
                live_keys.add(key)
...
def main(argv: list[str] | None = None) -> int:
    """Convert-phase entry point: translate ADF pipelines to IR (or merge agentic results).

    Exposed as a callable so the adapter can run the phase in-process instead of spawning a
    second interpreter.
    """
    ...
    # Translation IR is intermediate: write it to a transient .work/ subfolder; the prepare phase consumes
    # the report from there and prunes .work/, leaving metadata/ curated.
    ...
    # Write canonical translation_report.json so downstream tools (inspect, workspace-paths, dab_writer)
    # can reference a well-known filename regardless of --pipeline.
