...
def merge_bridge_requests(*requests: BridgeRequest | None) -> BridgeRequest | None:
    ...
    """
    populated = [request for request in requests if request is not None]
    if not populated:
        return None
    if len(populated) == 1:
        return populated[0]
    expression = " and ".join(f"({request.notebook_code})" for request in populated)
    imports: list[str] = []
...
def resolve_dict_values(fields: dict[str, Any] | None, context: TranslationContext) -> dict[str, str]:
    """Resolves all values in a dict that may contain ADF expressions.

    Args:
        fields: Dict of field name to raw values.
        context: Translation context for variable resolution.
    ...
    if not fields:
        return {}
    return {key: resolve_field(value, context) for key, value in fields.items()}
