...
from flowx.models.ir import Activity, LookupActivity, TranslationContext
...
def translate(
    ...
    # Resolve the Lookup's dataset reference: typeProperties.dataset is canonical; activity.inputs is the
    # legacy fall-back the loader uses for flattened activity shapes.
    ...
    if dataset_ref is not None:
        ...
        if dataset_type in _FILE_DATASET_TYPES:
            ...
            # Stash the dataset path components (container/folder/filename) for the code generator's
            # spark.read call. C-37 (LSC4-001): unwrap any ADF expression dict shapes to plain strings.
            ...
            # C-47 (LSC5-001): substitute dataset().X param refs from the dataset reference's bindings, then
            # resolve so the path is a real literal/{{job.parameters.X}} string, not a verbatim dataset() expr.
            ...
            # LSC3-005: surface the linked service URL when present so the generator can assemble the
            # abfss:// path for AzureBlobFS/ADLS-backed file datasets.
...
def _resolve_lookup_dataset(
    ...
    # Thread linkedService typeProperties.url onto the properties so the lookup notebook can assemble the
    # abfss:// file path for file-source datasets where the URL is only on the linked service.
