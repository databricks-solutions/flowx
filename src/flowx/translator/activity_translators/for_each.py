...
from flowx.models.ir import Activity, ForEachActivity, TranslationContext
from flowx.parser.expression_parser import resolve_expression
from flowx.translator.activity_translators.resolve import resolve_field_int
...
def translate(
    ...
    *,
    ...
    if expr_result is not None and expr_result.kind in ("dab_ref", "literal"):
        ...
        # C-31 (CF4-001): capture the resolved items notebook_code here while the full context is
        # available; the JSON-reload path has an empty variable_cache so it can't re-resolve @split(...).
