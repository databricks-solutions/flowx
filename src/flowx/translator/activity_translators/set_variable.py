...
from flowx.models.ir import Activity, SetVariableActivity, TranslationContext
...
def translate(
    ...
    # C-42 (VAREX5-001): a Set Pipeline Return Value activity carries a list of {key, value} pairs whose
    # inner value is a resolvable expression. The legacy path stringifies the whole list (then blanked by
    # the bundler), so unwrap a single pair's value and route it through normal resolution.
    ...
    if expr_result is not None:
        ...
        # C-33 (VAREX4-001/CF4-003): when the resolver can't handle the value, do NOT stamp
        # value_kind='literal' with raw @concat text (uninterpretable Python). Blank it and mark
        # unresolved so the bundler emits a manual_variable_init SetupTask.
        variable_value = ""
        ...
        if isinstance(value_raw, dict) and value_raw.get("type") == "Expression":
            ...
            # VAREX3-002: render Python bool as lowercase 'true'/'false' so @equals(variables('X'), true)
            # matches; str(True) would emit 'True' and silently invert the comparison.
    ...
    # Register variable -> task_key mapping in context. When the value is a DAB ref (e.g.
    # {{job.start_time.iso_datetime}} from @utcNow()), store it so downstream @variables() calls inline it.
