...
def translate(
    ...
    # C-28 (NB-ITER4-001): when notebookPath lowers to notebook_code (e.g. @trim(json(...).notebook_path)),
    # preserve the raw expression and mark the activity so the preparer emits a dispatch stub, not inline Python.
    ...
    # Resolve base_parameters at translate time (while variable_value_cache is available) so ADF
    # expressions inline to DAB refs; resolver caveats (e.g. utcnow()) go to parameter_approximations for SETUP.md.
...
# Library entry keys that may carry ADF expressions (jar/whl paths, maven coords with @concat); each is
# walked through the resolver and only emitted when every expression resolves to a clean literal/dab_ref.
    ...
    for lib in libraries:
        ...
        for key, value in lib.items():
            if key in _LIBRARY_VALUE_KEYS and isinstance(value, (str, dict)):
                result = resolve_expression(value, context)
                # C-13 (NB-ITER3-004): accept both literal and dab_ref so a jar path like
                # @pipeline().parameters.libName collapses to {{job.parameters.libName}}.
                if result is not None and result.kind in ("literal", "dab_ref"):
                    resolved_entry[key] = result.value
                else:
                    expression_text = _raw_expression_text(value)
                    resolved_entry[key] = value
                    # Only surface library entries whose value carried an ADF expression (starts with @);
                    # already-resolved literal paths need no SETUP.md callout.
