...
def _parse_condition(
    ...
    if m:
        ...
        if adf_op == "not":
            ...
            # C-15 (CF3-003/VAREX3-004): when the operand bridges to a Python bool task value, compare
            # against 'False' (not '') so the IfCondition can evaluate to FALSE.
    ...
    # Fallback truthy check. C-07: route an ADF function call through the bridge path so the operand
    # becomes a real task-value ref rather than legacy NOT_EQUAL '0' against a raw expression string.
    resolved, bridge = _resolve_operand(expr_str, context)
    if bridge is not None:
        return "NOT_EQUAL", _bridge_task_value_placeholder(), "False", bridge
    # C-15 (CF3-003/VAREX3-004): when the truthy operand is a task-value ref backed by a Python-bool
    # SetVariable (detected via a __BRIDGE__:: placeholder or 'true'/'false' body), compare against 'False'.
    if isinstance(resolved, str) and "__BRIDGE__" in resolved:
        return "NOT_EQUAL", resolved, "False", None
    # C-43 (CF5-001/LSC5-001): for a known-Boolean variable resolving to a parent-job task-value ref,
    # recompute the boolean locally via a BridgeRequest (mirroring Switch). Otherwise an inner-ForEach
    # IfCondition references a parent-only task, the bundler blanks the operand, and the true branch always runs.
    if _operand_is_known_boolean(expr_str, context):
        bridge = _boolean_variable_bridge(expr_str, resolved, context)
        if bridge is not None:
            return "NOT_EQUAL", _bridge_task_value_placeholder(), "False", bridge
        # C-32 (CF4-002): compare against lowercase 'false' (matching C-21 SetVariable rendering), not
        # legacy '0' which is always true for a Boolean-string operand (making the false branch dead code).
        return "NOT_EQUAL", resolved, "false", None
...
def _operand_is_known_boolean(expr: str, context: TranslationContext) -> bool:
    ...
    if var_match:
        ...
        if isinstance(cached, str) and cached.lower() in ("true", "false"):
            return True
        # C-41 (CF5-001): a Boolean variable seeded only by a literal default never caches as a
