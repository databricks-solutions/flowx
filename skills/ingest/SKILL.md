...
This is phase 1 of the flowx migration workflow. It takes raw ADF JSON exports and produces an `inventory.json` file that the `translate` skill consumes. The inventory classifies every ADF activity into one of three strategies:
...
```bash
python3 <plugin_dir>/src/flowx/parser/adf_loader.py \
