...
"<plugin_dir>/.venv/bin/python" <plugin_dir>/src/flowx/parser/adf_loader.py ...
```
...
```bash
python3 -m flowx.adapter inputs ingest
python3 -m flowx.adapter inputs translate
python3 -m flowx.adapter inputs prepare
```
...
| Output directory | Root directory for all flowx output | No | `./orchestra_output/` |
...
Invoke the `flowx:ingest` skill with the ADF source path and output directory set to `<output_dir>/ingest/`.
...
Invoke the `flowx:translate` skill with:
...
```bash
python3 -m flowx.adapter materialize-lookup "<csv-or-path>" \
...
```bash
python3 -m flowx.adapter inspect <output_dir>/translate/translation_report.json
```
...
```bash
python3 -m flowx.adapter modify \
...
```bash
python3 -m flowx.adapter workspace-paths \
...
3. Pass `--profile <name>` to the prepare invocation in Step 7 so
   flowx downloads the referenced notebooks and vendors them under
...
Invoke the `flowx:prepare` skill with:
