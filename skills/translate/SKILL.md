...
This is phase 2 of the flowx migration workflow. It consumes the `inventory.json` produced by the `ingest` skill and produces a `translation_report.json` that the `prepare` skill uses to generate Databricks Declarative Automation Bundles.
...
```bash
python3 <plugin_dir>/src/flowx/translator/engine.py \
...
Where:
- `<plugin_dir>` is the root of the flowx plugin
...
```bash
python3 -m flowx.adapter inspect <translation_report_path>
```
